...
Script.prototype.run = function (ctx, domain, fn) {
  ...
  try {
    if(ctx.server.options.env==="unsafe"){
                if(!context){
                        context = vm.createContext(scriptContext);
                }
                this.compiled.runInContext(context); //takes 2ms
    }else{
            this.compiled.runInNewContext(scriptContext); //takes 40ms
    }
